...
// supertest (a primeira versao deste teste, na rodada de correcao 1) nao
// provava nem isso direito: era capturado de forma sincrona, antes de
// qualquer I/O, entao so media quando a requisicao HTTP SAIU do cliente de
// teste — o que a propria estrutura do teste (tres promises criadas sem
// await entre si) ja garante por construcao.
//
// IMPORTANTE — o que este contador NAO prova (achado da rodada de correcao
// 2): gRPC sobre HTTP/2 multiplexa streams, entao ele so mede quantas
// chamadas AcceptLoad estao PENDENTES DE RESPOSTA a partir do gateway — nao
// quanto trabalho esta de fato em paralelo do outro lado. Ele e CEGO para
// qualquer serializacao que aconteca depois do disparo: dentro do proprio
// QuoteService.accept() no bidding, no pool de conexao do bidding, na ida
// ao catalog, no pool do catalog. Um revisor forcou o gateway a serializar
// accept() de ponta a ponta com uma fila/mutex no CONTROLLER do gateway, e
// este contador acusou (caiu para 1) — mas um mutex colocado so dentro do
// bidding deixa ESTE contador em 3 mesmo com o bidding processando uma
// aceitacao de cada vez. Por isso existe um segundo contador, irmao deste,
